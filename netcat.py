...
class NetCat:
    ...
    def handle(self,client_socket):
        if self.args.execute:
            ...
            while True:
                try:
                    client_socket.send(b'Noman:#>')
                    while '\n' not in cmd_buffer.decode():
                        cmd_buffer += client_socket.recv(64)
                        responses = execute(cmd_buffer.decode())
                        if responses:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Noman Net Tool',
                                     formatter_class=argparse.RawDescriptionHelpFormatter,
                                     epilog=textwrap.dedent('''Example: 
                                     netcat.py -t 192.168.1.108 -p 1234 -l -c # command shell
                                     netcat.py -t 192.168.1.108 -p 1234 -l 
                                     -u=mytest.txt #upload to file
                                     netcat.py -t 192.168.1.108 -p 1234 -l 
                                     -e=\"cat/tec/passwd\" # execute command
                                     echo 'ABC' | ./netcat.py -t 192.168.1.108 -p 135 # echo text to server port 135
                                     netcat.py -t 192.168.1.108 -p 1234 # connect to 
                                     server
                                     '''))
